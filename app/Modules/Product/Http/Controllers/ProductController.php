...
class ProductController extends Controller
{
  ...
  public function insert(Request $request)
  {
    ...
    return redirect('/admin/product/addproduct')->with('status', 'Product Added SuccessFully!!!!');
  }
...

  {
    ...
    if ($request->hasFile('image')) {
      ...
      $ext = $image->extension();
      $image_name = time() . '.' . $ext;
      // dd($image, public_path('product'));
      // $image->move('/product/', $image_name);
      $image->move(public_path('storage'), $image_name);
      // move_uploaded_file($image, '/product/'.$image_name);
      $product->image = $image_name;
    }
